...
Algún ejemplo que mapeo de teclas para .vimrc:

	" atajo para mostrar números de linea al pulsar F2:

	map <F2> <Esc>:set invnumber<CR>

	" atajo para mostrar caracteres no imprimibles al pulsar F3:

	map <F3> <Esc>:set invlist listchars=eol:$,tab:>-,trail:-<CR>

	" Activar el corrector ortográfico al pulsar F4
