...
$ curl -u usuario:clave --silent "https://mail.google.com/mail/feed/atom" | perl -ne 'print "t" if /<name>/; print "$2n" if /<(title|name)>(.*)</1>/;' (visualizar todos los mensajes no leidos de gmail)
